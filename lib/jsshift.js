...
function findExtendCallExpression(path) {
  let isExtend = p => p.node.type === 'CallExpression' &&
    (p.node.callee?.property?.name === 'extend' || (p.node.callee?.property?.name === 'create' && p.node.callee?.object?.name === 'Mixin')) &&
...
function addComment(j, path, comment) {
  let parent = findFirstParent( path,p=>p.node.type === 'ExportNamedDeclaration' || p.node.type === 'ExportDefaultDeclaration' ||
  p.node.type==='ReturnStatement' || p.node.type==='ExpressionStatement' || p.node.type==='VariableDeclaration' ||
  p.node.type==='AssignmentExpression');

  if (parent.parent && (parent.parent.node.type === 'ExportNamedDeclaration' || parent.parent.node.type === 'ExportDefaultDeclaration')) {
    parent = parent.parent;
  }

  if (parent) {
    parent.node.comments = parent.node.comments || [];
    if (!parent.node.comments.find(c => c.value.includes('TODO: intl-template-transform'))) {
      parent.node.comments.push(j.line(comment));
    }
  }else{
    throw new Error('Parent was not found, there must be p.node.type which needs to be added to first line of this function.');
  }
}
...
function createTransform(j, ast, onSomethingChanged) {
  return (path) => {
    ...
    const isCustomClass = !!findParent(path, p => p.node.key?.name === 'customClass');
    const isSizes = !!findParent(path, p => p.node.key?.name === 'sizes');
    const isStyles = !!findParent(path, p => p.node.key?.name === 'styles');
    const isCss = findParent(path, p => p.node.callee?.name === 'htmlSafe') && (findParent(path, p => p.node.key?.name?.includes('Logo')) || findParent(path, p => p.node.key?.name?.includes('Style')));
    const isCss2 = findParent(path, p => p.node.key?.name === 'table_class_list');

    if ((isTemplateTextToTranslate(path.node) || isText(path.node.value)) && !isPropertyName && !isSwitchCase && !isBinaryExpression &&
      !isResizablehandlers &&
      !isErrorHeader && !isFirstArgToFind && !isClassNames && !isError && !isFormatMessage &&
      !isExcludedFunctionCall(path.parent) && !isExcludedVariable(path.parent) && !isCustomClass &&
      !isSizes && !isStyles && !isCss && !isCss2
    ) {
      ...
      if (extend) {
        ...
        if (classBody) {
          ...
          if (funcDec) {
            ...
            if (argumentsChanged) {
              let trueParent = path.parent.parent;
              if (trueParent.node.type === 'VariableDeclarator') {
                trueParent = trueParent.parent;
              }

              if (trueParent.node.type === 'ObjectExpression') {
                trueParent = trueParent.parent.parent;
              }

              trueParent.node.comments = trueParent.node.comments || [];
              trueParent.node.comments.push(j.line('TODO: intl-template-transform function must have parameter intl, container, owner or intl can be defined inside function.', true, false));
              onSomethingChanged();
            } else {
            ...
            let trueParent = findParent(path, (p) => p.node.type === 'AssignmentExpression');

            if (!trueParent) {
              trueParent = findParent(path, (p) => p.node.type === 'VariableDeclarator');
              trueParent = trueParent.parent;
            }

            trueParent.parent.node.comments = trueParent.parent.node.comments || [];
            if (!trueParent.parent.node.comments.find(c => c.value.includes('TODO: intl-template-transform'))) {
              trueParent.parent.node.comments.push(j.line('TODO: intl-template-transform text must be inside ember class or inside a function with parameter intl, container, owner or intl can be defined inside the function.', true, false));
              onSomethingChanged();
            }
          }
        }
