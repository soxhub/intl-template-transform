...
function getComputedImport(j, ast) {
  ...
  if (importNode) {
    let spec = importNode.specifiers.find(is => is.imported?.name === 'computed');
    if (spec) {
      return spec.local?.name || 'computed';
    } else {
      //insert specifier
      importNode.specifiers.push(j.importSpecifier(j.identifier('computed')));
    }
  } else {
    //insert import
    const body = ast.find(j.Program).get('body');
...
function replaceTextWithComputed(j, path, ast) {
  let computedBody = j.blockStatement([
    j.returnStatement(createFormatMessageExpression(j, path.node))
  ]);
...
function shouldBeComputedProperty(callExpressionPath, propertyNode) {
  return callExpressionPath.node.arguments?.some(a => a.properties?.some(p => p === propertyNode)) ?? false;
}
...
function insertIntlToFunction(j, funcDec) {
  ...
  if (funcDec.node.params.find(p => p.name === 'container')) {
    ...
    // funcDec.node.params.unshift(j.identifier('intl'));
    return true;
  }
}
...
function createTransform(j, ast) {
  return (path) => {
    ...
    const isFirstArgToOn = path.parent.node.type === 'CallExpression' && path.parent.node.callee?.property?.name === 'on';
    const isFirstArgToIndexOf = path.parent.node.type === 'CallExpression' && path.parent.node.callee?.property?.name === 'indexOf';
    const isFirstArgToPluralize = path.parent.node.type === 'CallExpression' && path.parent.node.callee?.name === 'pluralize';
    ...
    const isFirstArgToFormat = path.parent.node.type === 'CallExpression' && path.parent.node.callee?.property?.name === 'format';
    ...
    const isFirstArgToAssert = path.parent.node.type === 'CallExpression' && path.parent.node.callee?.name === 'assert';
    const isClassNames = path.parent?.parent?.node?.key?.name === 'classNames';
    const isError = path.parent?.node?.callee?.name === 'Error';

    if ((isTemplateTextToTranslate(path.node) || isText(path.node.value)) && !isPropertyName && !isSwitchCase && !isFirstArgToPluralize && !isBinaryExpression &&
      !isFirstArgToOn && !isFirstArgToIndexOf && !isResizablehandlers && !isFirstArgToFormat &&
      !isErrorHeader && !isFirstArgToFind && !isFirstArgToAssert && !isClassNames && !isError && !isFormatMessage
    ) {
      let extend = findExtendCallExpression(path);
      if (extend) {
        if (shouldBeComputedProperty(extend, path.parent.node)) {
          addIntlService(j, ast, extend);
          replaceTextWithComputed(j, path, ast)
        } else {
          addIntlService(j, ast, extend);
          replaceText(j, path);
        }
        ...
        if (classBody) {
          ...
          if (funcDec) {
            ...
            // path.node.value = 't ' + path.node.value;
          }
        }
      }
    }
...
function transformer(file, api) {
  ...
  return ast2
    ...
    .forEach(createTransform(j, ast2))
    .toSource({
      quote: 'single',
      trailingComma: true,
      useTabs: true,
