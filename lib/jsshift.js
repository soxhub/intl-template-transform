...
function createTransform(j, ast, onSomethingChanged) {
  return (path) => {
    ...
    const isFormatMessage = path.parent.node.type === 'ObjectProperty' && path.parent?.parent?.parent?.node?.callee?.property?.name === 'formatMessage';
    ...
    const isCustomClass = findParent(path, p => [p.node.key?.name,p.node.id?.name].includes('customClass'));
    const isSizes = findParent(path, p => [p.node.key?.name,p.node.id?.name].includes('sizes'));
    const isStyles = findParent(path, p => [p.node.key?.name,p.node.id?.name].includes('styles'));
    ...
    const isCss2 = findParent(path, p => [p.node.key?.name,p.node.id?.name].includes('table_class_list'));
      ...
      if (extend) {
        ...
        if (classBody) {
          ...
          if (funcDec) {
            ...
            if (argumentsChanged) {
              addComment(j, path, 'TODO: intl-template-transform function must have parameter intl, container, owner or intl can be defined inside function.');
              onSomethingChanged();
            } else {
              ...
              addComment(j, path, 'TODO: intl-template-transform text must be inside ember class or inside a function with parameter intl, container, owner or intl can be defined inside the function.');
              onSomethingChanged();
          }
        }
      }
