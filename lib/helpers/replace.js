...
module.exports = function replace(group, serialized, isInsideStatement) {
  ...
  if (
    serialized.text.includes('>') ||
    serialized.text.includes('<') ||
    serialized.text.includes('&nbsp;') ||
    serialized.text.includes('&mdash;') ||
    serialized.text.includes('&middot;')
      ? true
      : false
  ) {
    params.push({
  ...
  let newLineAtStart =
    group[0].node.type === 'TextNode' && group[0].node.chars.match(/^(\s+)/);
  if (newLineAtStart) {
    newNodes.unshift({
      type: 'TextNode',
  ...
  let newLineAtEnd =
    group[group.length - 1].node.type === 'TextNode' &&
    group[group.length - 1].node.chars.match(/(\s+)$/);
  if (newLineAtEnd) {
    newNodes.push({
      type: 'TextNode',
