...
function* iteratePureNodes(node, parent, index) {
    let text = isText(node);
    let statement = isStatement(node);
...
function mustacheToString(node, isMustache) {
    return (isMustache ? '{' : '') + [
        node.path.original.replace(/\./g, '_'),
...
function musToString(node) {
    switch (node.type) {
        case 'PathExpression':
            return node.original.replace(/\./g, '_');
        case 'MustacheStatement':
        ...
        case 'StringLiteral':
            return node.value;
        case 'HashPair':
            return node.key + '_' + musToString(node.value);
    }
}
